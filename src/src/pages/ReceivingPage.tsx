...
import { getStationOptionsForDepartment } from "../data/structuredSelections";
...
export default function ReceivingPage({
  ...
  const activeCoveragePeople = loadCoveragePeople().filter((person) => person.status !== "OFFLINE");
  const requesterOptions = activeCoveragePeople.filter((person) => person.department === draft.requesterDepartment);
  const receivingWorkerOptions = activeCoveragePeople.filter((person) => person.department === "Receiving" || person.department === "Material Handling");
  const selectedWorker = receivingWorkerOptions.find((person) => person.name === workerName) ?? receivingWorkerOptions[0];
  const jobOptions = productionOrders.map((order) => `${order.orderNumber} - ${order.assemblyPartNumber}`);
  const destinationOptions = getStationOptionsForDepartment(draft.destinationDepartment);
  ...
  function submitOrder() {
    const normalizedDraft: ReceivingOrderDraft = {
      ...draft,
      poOrReceiver: draft.poOrReceiver || jobOptions[0] || '',
      orderedBy: draft.orderedBy || requesterOptions[0]?.name || '',
      destinationDetail: draft.destinationDetail || destinationOptions[0] || '',
    };

    if (
      !normalizedDraft.itemName.trim() ||
      !normalizedDraft.quantity.trim() ||
      !normalizedDraft.orderedBy.trim() ||
      !normalizedDraft.destinationDetail.trim() ||
      normalizedDraft.orderedBy === 'No signed-in requester'
    )
      return;
    const nextOrder = createReceivingOrder(normalizedDraft);
    setOrders((current) => [nextOrder, ...current]);
    setActiveId(nextOrder.id);
    setDraft(createBlankDraft(normalizedDraft.requesterDepartment));
    setSubmitMessage(
        ...
        <section style={getPanelStyle(theme)}>
          ...
          <div style={getFormGridStyle()}>
            ...
            <SelectField
              ...
              value={draft.poOrReceiver || jobOptions[0] || ""}
              options={jobOptions.length > 0 ? jobOptions : ["No active orders"]}
              onChange={(value) => setDraft({ ...draft, poOrReceiver: value })}
              theme={theme}
            />
            <SelectField
              label="Requested by"
              value={draft.orderedBy || requesterOptions[0]?.name || ""}
              options={requesterOptions.length > 0 ? requesterOptions.map((person) => person.name) : ["No signed-in requester"]}
            ...
            <SelectField
              ...
              onChange={(value) => {
                ...
                setDraft({
                  ...draft,
                  destinationDepartment: department,
                  destinationDetail: getStationOptionsForDepartment(department)[0],
                });
              }}
              theme={theme}
            />
            <SelectField
              ...
              value={draft.destinationDetail || destinationOptions[0] || ""}
              options={destinationOptions}
              onChange={(value) => setDraft({ ...draft, destinationDetail: value })}
              theme={theme}
            />
            <Field
            ...
            <section style={getPanelStyle(theme)}>
              ...
              <div style={getFormGridStyle()}>
                <SelectField
                  ...
                  value={workerName || selectedWorker?.name || ""}
                  options={receivingWorkerOptions.length > 0 ? receivingWorkerOptions.map((person) => person.name) : ["No signed-in receiving worker"]}
              ...
              <div style={buttonRowStyle}>
                <button
                  onClick={() =>
                    updateOrder(
                      checkInReceivingOrder(activeOrder, selectedWorker?.name ?? "Receiving worker", notes),
                    )
                  }
                ...
                <button
                  onClick={() =>
                    updateOrder(claimReceivingDelivery(activeOrder, selectedWorker?.name ?? "Receiving worker"))
                  }
                ...
                <button
                  onClick={() =>
                    updateOrder(
                      deliverReceivingOrder(activeOrder, selectedWorker?.name ?? "Receiving worker", notes),
                    )
                  }
...
function loadCoveragePeople(): CoveragePerson[] {
  try {

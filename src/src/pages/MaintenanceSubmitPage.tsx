...
import { maintenanceIssueOptions, isOtherDescriptionValid, OTHER_DESCRIPTION_MAX, OTHER_DESCRIPTION_MIN } from '../data/structuredSelections';
...
export default function MaintenanceSubmitPage({
  ...
  const [issueCode, setIssueCode] = useState(maintenanceIssueOptions[0].code);
  ...
  const submitterOptions = loadCoveragePeople().filter((person) => person.status !== 'OFFLINE');
  const selectedSubmitter = submitterOptions.find((person) => person.id === submittedById) ?? submitterOptions[0];
  const selectedIssue = maintenanceIssueOptions.find((issue) => issue.code === issueCode) ?? maintenanceIssueOptions[0];

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();

    const requiresOther = selectedIssue.code === 'OTHER';
    if (!machineId || !selectedSubmitter || (requiresOther && !isOtherDescriptionValid(otherDescription))) {
      alert(requiresOther ? `Other description must be ${OTHER_DESCRIPTION_MIN}-${OTHER_DESCRIPTION_MAX} characters.` : 'Please fill in all required selections');
      return;
    }

    const selectedTarget = serviceTargets.find((target) => target.id === machineId);
    if (!selectedTarget) return;
    const problem = requiresOther ? `Other: ${otherDescription.trim()}` : selectedIssue.label;
    const submittedBy = `${selectedSubmitter.name} (${selectedSubmitter.role} / ${selectedSubmitter.department})`;
    const newRequest = {
      id: `REQ-${Date.now()}`,
      machineId,
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} style={getFormStyle(theme)}>
        ...
        <div style={fieldStyle}>
          <label style={getLabelStyle(theme)}>Issue Type *</label>
          <select
            value={issueCode}
            onChange={(e) => setIssueCode(e.target.value as typeof issueCode)}
            style={getSelectStyle(theme)}
            required
          >
            {maintenanceIssueOptions.map((issue) => (
        ...
        {issueCode === 'OTHER' ? (
          <div style={fieldStyle}>
            <label style={getLabelStyle(theme)}>Other Description *</label>
            <input
              type="text"
              value={otherDescription}
              onChange={(e) => setOtherDescription(e.target.value.slice(0, OTHER_DESCRIPTION_MAX))}
              style={getInputStyle(theme)}
              placeholder={`Required, ${OTHER_DESCRIPTION_MIN}-${OTHER_DESCRIPTION_MAX} characters`}
              minLength={OTHER_DESCRIPTION_MIN}
        ...
        <div style={fieldStyle}>
          <label style={getLabelStyle(theme)}>Submitted By *</label>
          <select
            value={submittedById || selectedSubmitter?.id || ''}
          ...
          >
            {submitterOptions.map((person) => (
              <option key={person.id} value={person.id}>
                {person.name} ({person.role} · {person.department} · {person.status})
              </option>
            ))}
          </select>
        </div>

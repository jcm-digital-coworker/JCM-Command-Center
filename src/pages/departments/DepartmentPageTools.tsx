...
export function Section({ title, children, theme = 'dark' }: DepartmentPageProps & { title: string; children: ReactNode }) {
  return (
...
export function AssetCard({ asset, theme = 'dark' }: DepartmentPageProps & { asset: PlantAsset }) {
  return (
...
export function OrderCard({ order, theme = 'dark' }: DepartmentPageProps & { order: ProductionOrder }) {
  return (
    <div style={cardStyle(theme)}>
      <div style={cardHeaderStyle}>
  ...
  return (
    <CardGrid>
      {recommendations.map((recommendation) => (
        <div key={recommendation.id} style={crewCardStyle(recommendation.tone, theme)}>
          <div style={cardHeaderStyle}>
            <strong style={cardTitleStyle(theme)}>{recommendation.title}</strong>
            <span style={crewToneStyle(recommendation.tone, theme)}>{recommendation.tone}</span>
          </div>
          <p style={bodyStyle(theme)}>{recommendation.reason}</p>
          <div style={chipRowStyle}>
            {recommendation.suggestedRoles.map((role) => (
              <span key={role} style={chipStyle(theme)}>Role: {role}</span>
            ))}
            {recommendation.orderNumbers.length > 0 ? recommendation.orderNumbers.map((orderNumber) => (
              <span key={orderNumber} style={chipStyle(theme)}>Order: {orderNumber}</span>
            )) : <span style={chipStyle(theme)}>No target order</span>}
          </div>
          <div style={workerListStyle(theme)}>
            <strong>Available co-workers:</strong>{' '}
            {recommendation.availableWorkers.length > 0
              ? recommendation.availableWorkers.map((worker) => worker.name).join(', ')
...
function heroStyle(theme: 'dark' | 'light'): CSSProperties {
  return {
    ...
    borderRadius: 6,
    padding: 18,
    boxShadow: theme === 'dark' ? '0 8px 24px rgba(0,0,0,0.25)' : '0 4px 14px rgba(15,23,42,0.08)',
  };
}

function eyebrowStyle(theme: 'dark' | 'light'): CSSProperties {
  return { color: '#f97316', fontSize: 11, fontWeight: 900, letterSpacing: '1px', textTransform: 'uppercase' };
}

function titleStyle(theme: 'dark' | 'light'): CSSProperties {
  return { margin: '6px 0', color: theme === 'dark' ? '#e2e8f0' : '#0f172a', fontSize: 24, letterSpacing: '0.5px' };
}

function subtitleStyle(theme: 'dark' | 'light'): CSSProperties {
  return { margin: 0, color: theme === 'dark' ? '#94a3b8' : '#475569', lineHeight: 1.5, fontSize: 14 };
}
...
function sectionTitleStyle(theme: 'dark' | 'light'): CSSProperties {
  return { margin: '0 0 12px 0', color: theme === 'dark' ? '#f8fafc' : '#0f172a', fontSize: 15, textTransform: 'uppercase', letterSpacing: '0.8px' };
}
...
const cardHeaderStyle: CSSProperties = { display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-start' };
...
function metaStyle(theme: 'dark' | 'light'): CSSProperties {
  return { color: theme === 'dark' ? '#94a3b8' : '#64748b', fontSize: 12, marginTop: 6, fontWeight: 700 };
}

function bodyStyle(theme: 'dark' | 'light'): CSSProperties {
  return { color: theme === 'dark' ? '#cbd5e1' : '#334155', fontSize: 13, lineHeight: 1.45 };
}

function pillStyle(status: string, theme: 'dark' | 'light'): CSSProperties {
  const danger = ['DOWN', 'BLOCKED', 'FAILED', 'HOLD'].includes(status);
  const warn = ['WATCH', 'UNKNOWN', 'PARTIAL', 'PENDING', 'WAITING'].includes(status);
...
const chipRowStyle: CSSProperties = { display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 };
...
const gridStyle: CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 10 };

function crewCardStyle(tone: 'GO' | 'WATCH' | 'HOLD', theme: 'dark' | 'light'): CSSProperties {
...
function crewToneStyle(tone: 'GO' | 'WATCH' | 'HOLD', theme: 'dark' | 'light'): CSSProperties {
  const color = tone === 'GO' ? '#10b981' : tone === 'HOLD' ? '#ef4444' : '#f59e0b';
  return {
    border: `1px solid ${color}`,
    color,
...
function emptyStyle(theme: 'dark' | 'light'): CSSProperties {
  return { color: theme === 'dark' ? '#94a3b8' : '#64748b', fontSize: 13, padding: 12, border: theme === 'dark' ? '1px dashed #334155' : '1px dashed #cbd5e1', borderRadius: 6 };
}
